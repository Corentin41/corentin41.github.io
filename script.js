...
function getAge() {
  ...
  if ((currentMonth == 2 && currentDay >= 25) || currentMonth >= 2) {
...
elementsRight.forEach((element) => {
  new IntersectionObserver(startAnimation("slide-in-from-right")).observe(
    element,
    options
  );
});
...
elementsLeft.forEach((element) => {
  new IntersectionObserver(startAnimation("slide-in-from-left")).observe(
    element,
    options
  );
});
...
elementsPop.forEach((elPop) => {
  new IntersectionObserver(startAnimation("pop_animation")).observe(
    elPop,
    options
  );
});
...
elementsSkillsPop.forEach((element) => {
  new IntersectionObserver(startAnimation("skill_pop_animation")).observe(
    element,
    options
  );
});
...
function popAnimationDelay() {
  ...
  skills.forEach((box) => {
    ...
    let pop_delay = getComputedStyle(document.documentElement).getPropertyValue(
      "--pop-delay"
    );
...
function increaseDelay(pop_delay) {
  ...
  pop_delay = getComputedStyle(document.documentElement).getPropertyValue(
    "--pop-delay"
  );
